...
//- 注意无论用户是否被逻辑删除都应该被被查到
    ...
    public Result<?> assign(@RequestParam Long roleId, @RequestParam Long[] permissionIds){
...
//- 如果角色被删除则拒绝执行接口
    ...
    public Result<?> add(@RequestParam String name,@RequestParam String permissionValue){
//        添加权限
        throw new CustomException(ResultCodeEnum.FUNCTION_ERROR);
    }
    ...
    public Result<?> update(@RequestParam String name,@RequestParam String permissionValue){
//        更新权限信息,权限被逻辑删除则拒绝更新
        throw new CustomException(ResultCodeEnum.FUNCTION_ERROR);
    }
