...
- **接口权限**:任何用户可访问
- **接口描述**: 商户端的注册接口,
- 要判断用户名,手机号或邮箱是否已存在,如果都为null则注册成功,
- 然后分配ROLE_NEW_MERCHANT角色,创建user表,关联user_role表,创建user_detail表,创建merchant_supplement表
- 如果存在不指向同一个用户的情况则返回错误,
- 如果指向同一个用户则判断有没有商户相关角色,如果有则返回错误,否则分配ROLE_NEW_MERCHANT角色
- **接口实现**:得到参数后分别根据用户名,手机号和邮箱获取user表的一条数据,
- 如果三份数据都是null则注册用户,分配ROLE_NEW_MERCHANT角色,创建user表,关联user_role表,创建user_detail表,创建merchant_supplement表并填充默认数据
...
- **接口描述**: 实现了对单个用户的user表,user_detail表,merchant_supplement表的单条或单条字段的数据的更新,但更新某些字段需要验证参数
- **接口实现**: 
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
