...
- 如果是管理员查看用户详细信息则额外携带List<userAddressVO>,MerchantDetailVO,对敏感信息进行脱敏处理
...
- **接口实现**: 商品被逻辑删除则拒绝更新,查询到的商品存在不合法,不匹配的情况则拒绝更新
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
- **请求方法**: PUT
- **接口权限**:(需要product.delete权限而且访问者的id等于参数的userId),或者(需要product.delete权限而且访问者是管理员(但非默认角色))
- **接口描述**: 删除商品和商品的规格,回收优惠券,回收用户持有的优惠券
...
- **接口权限**: 任何用户可访问
- **接口描述**: 在用户端首页推送商品
- **接口实现**: 商品推送可用用机器学习实现也可用基于规则的算法实现,但这里不实现个性化推荐,采用简单推送.
...
|--------------|---------|---------|-------|----|
| pageNum      | 否(默认1)  | Integer | 页码    | 1  |
| pageSize     | 否(默认20) | Integer | 每页数据量 | 10 |
| categoryCode | 否(默认1)  | Long    | 分类标识  | 1  |
| search       | 否(默认"") | string  | 关键词   | "" |
| region       | 否(默认"") | string  | 地区    | "" |
| sort         | 否(默认"") | string  | 排序字段  | "" |
...
|------------|------|---------|--------|-------|
| id         | 是    | Integer | id     | 1     |
| name       | 否    | string  | 商品分类名  | 文具    |
| code       | 否    | string  | 商品分类编码 | 1001  |
| createTime | 否    | Date    | 创建时间   | null  |
| updateTime | 否    | Date    | 修改时间   | null  |
| deleted    | 否    | boolean | 逻辑删除   | false |
...
- 不能删除"默认商品分类",然后遍历商品,如果商品有该商品分类则将该商品分类变成"默认商品分类"
- 只有超级管理员才可以删除商品分类
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
...
- **请求方法**: POST
- **接口权限**:需要productSku.add权限而且而且访问者的id等于参数的userId
- **接口描述**: 只可在商户在创建商品后编辑商品时可用
- **接口实现**: 添加商品规格后,要刷新商品的库存,注意商品规格要指向合法的商品
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
|------------|------|------------|---------|-------|
| id         | 否    | Integer    | id      | null  |
| productId  | 是    | string     | 绑定的商品id | 1     |
| specs      | 是    | string     | 商品标识    | 默认款   |
| price      | 是    | BigDecimal | 价格      | 10.00 |
| stock      | 是    | string     | 商品分类名   | 文具    |
| sku_code   | 是    | string     | 商品分类名   | 文具    |
| logo       | 是    | string     | 商品分类编码  | 1001  |
| createTime | 否    | Date       | 创建时间    | null  |
| updateTime | 否    | Date       | 修改时间    | null  |
| deleted    | 否    | boolean    | 逻辑删除    | false |
...
- **请求方法**: PUT
- **接口权限**:需要productSku.update权限而且而且访问者的id等于参数的userId
- **接口描述**: 只可在商户在创建商品后编辑商品时可用
- **接口实现**: 更新商品规格后,要刷新商品的库存,注意商品规格要指向合法的商品
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
|------------|------|------------|---------|-------|
| id         | 是    | Integer    | id      | 1     |
| productId  | 否    | string     | 绑定的商品id | 1     |
| specs      | 否    | string     | 商品标识    | 默认款   |
| price      | 否    | BigDecimal | 价格      | 10.00 |
| stock      | 否    | string     | 商品分类名   | 文具    |
| sku_code   | 否    | string     | 商品分类名   | 文具    |
| logo       | 否    | string     | 商品分类编码  | 1001  |
| createTime | 否    | Date       | 创建时间    | null  |
| updateTime | 否    | Date       | 修改时间    | null  |
| deleted    | 否    | boolean    | 逻辑删除    | false |
...
- **请求方法**: PUT
- **接口权限**: 需要productSku.delete权限而且访问者的id等于参数的userId
- **接口描述**: 删除商品的规格
- **接口实现**: 删除商品的规格,但不能删除该商品的最后一个商品规格,
- 也不能删除商品的默认规格,如果规格无法指向具体商品则在删除后报错,删除后修改商品库存
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
- **接口描述**: 在后台和商户端的优惠券管理界面查看平台的所有优惠券或当前商户的优惠券,或在商品管理界面查看商品可用的优惠券
- **接口实现**: 在后台和商户端的商品管理界面查看所有存在的优惠券或当前商户的优惠券,
...
| pageNum   | 否(默认1)  | Integer | 页码    | 1  |
| pageSize  | 否(默认10) | Integer | 每页数据量 | 10 |
| search    | 否(默认空)  | String  | 关键字搜索 | "" |
| userId    | 否(默认0)  | Long    | 用户id  | 0  |
| productId | 否(默认0)  | Long    | 商品id  | 0  |
...
- **接口描述**: 添加优惠券
- **接口实现**: 添加优惠券,如果targetId有效须判断指向对象是否没有被删除
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
|----------------|---------------|------------|----------------------|---------|
| id             | 否             | Integer    | id                   | null    |
| name           | 是             | string     | 优惠券名                 | 中秋节全场福利 |
| getType        | 是             | integer    | 0-满减 1-折扣            | 0       |
| targetType     | 是             | Integer    | 0-全场通用 1-指定分类 2-指定商品 | 0       |
| merchantId     | 否(指定商品的优惠券激活) | Long       | 商品指向的商户id            | 0       |
| isDiscount     | 是             | Integer    | 0-通用 1-商品优惠时不可用      | 0       |
| targetId       | 否(默认0)        | Long       | 优惠券目标ID,无指定为0        | 0       |
| threshold      | 是             | BigDecimal | 使用门槛                 | 200     |
| value          | 是             | BigDecimal | 优惠值(满减时为金额,折扣时为折扣)   | 30      |
| startTime      | 是             | Date       | 生效时间                 | xxx     |
| endTime        | 是             | Date       | 失效时间                 | xxx     |
| stock          | 是             | Integer    | 发放总量                 | 100     |
| remainingStock | 否             | Integer    | 剩余总量                 | 0       |
| status         | 否             | Integer    | 0-未生效 1-生效中 2-已过期    | 0       |
| createTime     | 否             | Date       | 创建时间                 | null    |
| updateTime     | 否             | Date       | 修改时间                 | null    |
| deleted        | 否             | boolean    | 逻辑删除                 | false   |
...
- **接口描述**: 修改优惠券
- **接口实现**: 修改优惠券,如果targetId有效须判断指向对象是否没有被删除
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
|----------------|---------------|------------|----------------------|---------|
| id             | 否             | Integer    | id                   | null    |
| name           | 否             | string     | 优惠券名                 | 中秋节全场福利 |
| getType        | 否             | integer    | 0-满减 1-折扣            | 0       |
| targetType     | 否             | Integer    | 0-全场通用 1-指定分类 2-指定商品 | 0       |
| merchantId     | 否(指定商品的优惠券激活) | Long       | 商品指向的商户id            | 0       |
| isDiscount     | 否             | Integer    | 0-通用 1-商品优惠时不可用      | 0       |
| targetId       | 否(默认0)        | Long       | 优惠券目标ID,无指定为0        | 0       |
| threshold      | 否             | BigDecimal | 使用门槛                 | 200     |
| value          | 否             | BigDecimal | 优惠值(满减时为金额,折扣时为折扣)   | 30      |
| startTime      | 否             | Date       | 生效时间                 | xxx     |
| endTime        | 否             | Date       | 失效时间                 | xxx     |
| stock          | 否             | Integer    | 发放总量                 | 100     |
| remainingStock | 否             | Integer    | 剩余总量                 | 0       |
| status         | 否             | Integer    | 0-未生效 1-生效中 2-已过期    | 0       |
| createTime     | 否             | Date       | 创建时间                 | null    |
| updateTime     | 否             | Date       | 修改时间                 | null    |
...
- **接口描述**: 删除优惠券
- **接口实现**: 删除优惠券,删除时回收用户领取的优惠券,如果优惠券不是全场通用缺没有绑定合法的商户或商品则删除后报错.不能重复删除
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
### 用户领取的优惠券相关接口 UserCouponController

#### 查询某用户领取的优惠券列表接口
##### 基础信息
- **接口名称**: 查询某用户领取的优惠券列表
- **接口路径**: `/api/product/userCoupon/personList`
- **请求方法**: GET
- **接口权限**: 需要userCoupon.list权限且访问者的id等于参数的userId
- **接口描述**: 在用户端的个人中心的优惠券界面查看自己领取的优惠券
- **接口实现**: 查看自己所有的已领取的未过期未被删除且未被使用的优惠券,
- 要保证用户领取的优惠券指向的优惠券合法
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
...
##### 基础信息
- **接口名称**: 查询某用户领取的优惠券列表
- **接口路径**: `/api/product/userCoupon/productList`
- **请求方法**: GET
- **接口权限**: 需要userCoupon.list权限且访问者的id等于参数的userId
- **接口描述**: 访问商品时查看对于该商品,自己有什么可用的优惠券,被查看商品详情接口调用
- **接口实现**: 查看自己所有的已领取的未过期未被删除且未被使用的优惠券,
...
| 参数名       | 是否必填 | 数据类型 | 说明         | 示例 |
|-----------|------|------|------------|----|
| productId | 是    | Long | 商品id       | 1  |
| targetId  | 是    | Long | 优惠券指向的商户id | 1  |
...
##### 基础信息
- **接口名称**: 添加用户领取的优惠券
- **接口路径**: `/api/product/userCoupon/add`
- **请求方法**: POST
- **接口权限**: 需要userCoupon.add权限
- **接口描述**: 用户进行领取优惠券操作
...
|------------|------|---------|-------|-------|
| id         | 否    | Long    | id    | null  |
| couponId   | 是    | Long    | 优惠券id | 1     |
| userId     | 否    | Long    | 用户id  | 1     |
| getTime    | 否    | Date    | 领取时间  | null  |
| useTime    | 否    | Date    | 使用时间  | null  |
| orderId    | 否    | Long    | 订单id  | null  |
| status     | 否    | Integer | 订单状态  | 0     |
| createTime | 否    | Date    | 创建时间  | null  |
| updateTime | 否    | Date    | 修改时间  | null  |
| deleted    | 否    | boolean | 逻辑删除  | false |
...
##### 基础信息
- **接口名称**: 消费优惠券
- **接口路径**: `/api/product/userCoupon/consume`
- **请求方法**: POST
- **接口权限**: 需要userCoupon.update权限
- **接口描述**: 用户在下单时进行优惠券的消费
...
|------------|------|---------|-------|-------------|
| id         | 否    | Long    | id    | null        |
| couponId   | 否    | Long    | 优惠券id | null        |
| userId     | 否    | Long    | 用户id  | null        |
| getTime    | 否    | Date    | 领取时间  | null        |
| useTime    | 否    | Date    | 使用时间  | null        |
| orderId    | 是    | Long    | 订单id  | 11213142153 |
| status     | 否    | Integer | 订单状态  | 0           |
| createTime | 否    | Date    | 创建时间  | null        |
| updateTime | 否    | Date    | 修改时间  | null        |
| deleted    | 否    | boolean | 逻辑删除  | false       |
...
##### 基础信息
- **接口名称**: 回收用户领取的优惠券
- **接口路径**: `/api/product/userCoupon/destroy`
- **请求方法**: POST
- **接口权限**: 需要userCoupon.delete权限
- **接口描述**: 由于一些原因,触发优惠券的删除,这时会回收用户领取的优惠券,在优惠券被删除时调用接口
- **接口实现**: 批量逻辑删除该couponId指向的所有用户领取的优惠券,然后减去优惠券的库存
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
- **接口权限**: 需要order.add权限
- **接口描述**: 用户进行下单操作
- **接口实现**: 得到参数后将token信息和userId比对并检查用户是否可以正常使用,
...
- 携带的用户信息是商家信息
- 返回List<OrdertVO>并由PageResult包装
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
##### 基础信息
- **接口名称**: 订单支付
- **接口路径**: `/api/order/orderStatusLog/pay`
- **请求方法**: POST
- **接口权限**:需要orderStatusLog.add权限
- **接口描述**: 用户支付订单
- **接口实现**: 检查订单是否处于 "待支付" 状态,直接生成order_status_log表,
...
##### 基础信息
- **接口名称**: 订单发货
- **接口路径**: `/api/order/orderStatusLog/ship`
- **请求方法**: POST
- **接口权限**:需要orderStatusLog.add权限
- **接口描述**: 商家为订单发货
- **接口实现**: 检查订单是否处于 "已支付" 状态,直接生成order_status_log表,然后调用修改订单接口,解决幂等性问题
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
##### 基础信息
- **接口名称**: 订单收货
- **接口路径**: `/api/order/orderStatusLog/receive`
- **请求方法**: POST
- **接口权限**:需要orderStatusLog.add权限
- **接口描述**: 用户为订单收货
- **接口实现**: 检查订单是否处于 "已发货" 状态,直接生成order_status_log表,然后调用修改订单接口,解决幂等性问题
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
...
##### 基础信息
- **接口名称**: 取消订单
- **接口路径**: `/api/order/orderStatusLog/cancel`
- **请求方法**: POST
- **接口权限**:需要orderStatusLog.add权限
- **接口描述**: 订单超时为支付时触发
- **接口实现**: 检查订单是否处于 "待支付" 状态,直接生成order_status_log表,
...
- **接口实现**: 检查订单是否不是处于 "待支付" 状态,直接生成order_status_log表,
...
##### 基础信息
- **接口名称**: 列出订单状态
- **接口路径**: `/api/order/orderMain/orderStatusLog/{id}`
- **请求方法**: GET
- **接口权限**: 需要orderStatusLog.list权限
- **接口描述**: 查看订单的详细信息时调用该接口
- **接口实现**: 查看订单的详细信息时调用该接口
- **接口备注**: 用户端不可访问
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
