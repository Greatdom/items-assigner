...
| client(String)  | 否(前端传递)   | string | 客户端名称                       | 后台端                                       |
...
- **请求方法**: GET
- **接口权限**:需要user.list权限或者参数的id等于访问者的id
...
- 如果是管理员查看用户详细信息则额外调用获取用户的地址簿接口,携带List<userAddressDTO>,MerchantDetailVO,对敏感信息进行脱敏处理
...
```json
{
  ...
  "data": {
    ...
    "userAddressDTO(List<UserAddressDTO>)": [
      {
        "id": 1,
        "receiver": "张三",
        "phone": "156****8742",
...
- **接口权限**: 访问者的id等于参数的id
- **接口描述**: 获取个人用户地址簿
- **接口实现**: 在用户端个人中心或后台的用户管理可查询用户端地址簿,返回List<UserAddress>
- 只需返回没有被逻辑删除的地址
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
...
##### 基础信息
- **接口名称**: 开张/关店
- **接口路径**: `/api/user/merchantSupplement/status/{id}`
- **请求方法**: PUT
- **接口权限**: 需要user.update权限或访问者的id等于参数id
- **接口描述**: 开张/关店
- **接口实现**: 传入用户id,开张/关店,此时用户无法在这家店发起订单并强制取消或退货还没有完成的订单
- 查询不到用户或用户被逻辑删除则跳过
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
...
- **请求方法**: PUT
- **接口权限**: 需要product.update权限而且访问者的id等于参数的userId
    ...
    "records": [
      {
        ...
        "pointer": "优惠券指向的商户昵称或标明公用优惠券或商品名称"
      }
    ],
    "total": 1,
    "size": 10,
...
- **请求方法**: GET
- **接口权限**: 需要order.list权限而且访问者的id等于参数的userId而且是商户
...
##### 基础信息
- **接口名称**: 查看订单详细信息
- **接口路径**: `/api/order/orderMain/detail/{id}`
- **请求方法**: GET
...
- **请求方法**: POST
- **接口权限**: 需要orderStatusLog.add权限且访问者的id等于订单的userId
- **接口描述**: 用户支付订单
...
- **请求方法**: POST
- **接口权限**: 需要orderStatusLog.add权限且访问者的id等于订单的userId且是商家
- **接口描述**: 商家为订单发货
...
- **请求方法**: GET
- **接口权限**: 需要financialFlow.list权限
- **接口描述**: 后台端在订单管理页面查看网站的所有财务列表
- **接口实现**: 返回List<FinancialFlow>并由PageResult包装
##### 请求参数
###### 请求头
| 参数名          | 是否必填 | 数据类型   | 说明    | 示例                                      |
|--------------|------|--------|-------|-----------------------------------------|
